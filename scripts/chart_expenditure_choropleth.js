(function () {
    const svg = d3.select("#expenditure-choropleth-map");
    ...
    Promise.all([
        ...
        const select = d3.select("#expenditure-choropleth-year-select");
        select.selectAll("option").remove();
        years.forEach(y => select.append("option").attr("value", y).text(y));
        select.property("value", years[years.length - 1]);

        const tooltip = d3.select("#expenditure-choropleth-tooltip");

        function update(year) {
            const vals = Object.values(spendByCode)
                .map(o => o[year])
            ...
            gEU.selectAll("path")
                .on("mouseover", function (event, d) {
                    ...
                    tooltip.style("display", "block")
                        .text(`${codeToName[code] || code} (${year}): ${v ? "€" + v.toFixed(0) + " per trip" : "no data"}`);
                })
                .on("mousemove", function (event) {
                    const c = document.querySelector("#expenditure-choropleth-map").getBoundingClientRect();
                    tooltip
        ...
        function drawLegend(scale) {
            ...
            const grad = defs.append("linearGradient").attr("id", "expenditure-cbar");
            ...
            legendG.append("rect")
                ...
                .attr("fill", "url(#expenditure-cbar)")
