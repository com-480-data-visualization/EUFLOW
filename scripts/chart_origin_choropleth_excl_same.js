(function () {
    const svg = d3.select("#origin-choropleth-excl-map");
    ...
    // Curated palette; same scheme as the first origin choropleth so the two maps read together
    const originColor = {
    ...
    Promise.all([
        ...
        const select = d3.select("#origin-choropleth-excl-year-select");
        ...
        const tooltip = d3.select("#origin-choropleth-excl-tooltip");
            ...
            gEU.selectAll("path")
                ...
                .on("mouseover", function (event, d) {
                    ...
                    tooltip.style("display", "block")
                        .text(`${destName} (${year}) — top foreign origin: ${originName}`);
                })
                .on("mousemove", function (event) {
                    const container = document.querySelector("#origin-choropleth-excl-map").getBoundingClientRect();
                    tooltip
