(function () {
    const svg = d3.select("#nights-heatmap-map");
    ...
    Promise.all([
        ...
        // Build lookup nights[geo][year][month] = total
        const nights = {};
        ...
        csv.forEach(row => {
            ...
            const v     = +row.nights;
            if (!geo || !month || !year || !isFinite(v)) return;
            yearSet.add(year);
            if (!nights[geo]) nights[geo] = {};
            if (!nights[geo][year]) nights[geo][year] = {};
            nights[geo][year][month] = v;
        });
        ...
        // World backdrop (gray non-EU countries for context)
        ...
        const ySelect = d3.select("#nights-heatmap-year-select");
        ...
        const mSelect = d3.select("#nights-heatmap-month-select");
        mSelect.selectAll("option").remove();
        monthOrder.forEach(m => mSelect.append("option").attr("value", m).text(m));
        mSelect.property("value", "August");

        const tooltip = d3.select("#nights-heatmap-tooltip");

        const cbarG = svg.append("g").attr("class","cbar");

        function update() {
            const year  = ySelect.property("value");
            const month = mSelect.property("value");

            // Build the value set for the scale from NUTS regions only
            const vals = [];
            nutsFeats.forEach(f => {
                const code = f.properties.NUTS_ID;
                const v = nights[code] && nights[code][year] && nights[code][year][month];
            ...
            gLayer.selectAll("path")
                .on("mouseover", function (event, d) {
                    ...
                    tooltip.style("display", "block")
                        .text(v && v > 0
                            ? `${name}, ${month} ${year}: ${fmtNights(v)}`
                            : `${name}, ${month} ${year}: no data`);
                })
                .on("mousemove", function (event) {
                    const c = document.querySelector("#nights-heatmap-map").getBoundingClientRect();
                    tooltip
        ...
        function drawColorBar(scale, year, month) {
            cbarG.selectAll("*").remove();
            const lw = 220, lh = 12, lx = 24, ly = 640;

            const defs = cbarG.append("defs");
            const grad = defs.append("linearGradient").attr("id", "nights-heatmap-cbar");
